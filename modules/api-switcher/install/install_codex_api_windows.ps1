...
Copy-Item "$ModuleRoot\bin\codex_api" $TargetScript -Force
Copy-Item "$ModuleRoot\bin\codex_api.bat" $TargetBat -Force
python "$ModuleRoot\lib\merge_json_template.py" $TargetAuthList "$ModuleRoot\templates\codex\auth_list.json"

python "$ModuleRoot\lib\merge_codex_config.py" $TargetConfig "$ModuleRoot\templates\codex\config.providers.toml"
