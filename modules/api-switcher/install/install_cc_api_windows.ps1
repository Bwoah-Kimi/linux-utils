...
Copy-Item "$ModuleRoot\bin\cc_api" $TargetScript -Force
Copy-Item "$ModuleRoot\bin\cc_api.bat" $TargetBat -Force
python "$ModuleRoot\lib\merge_json_template.py" $TargetProviderList "$ModuleRoot\templates\claude\provider_list.json"
